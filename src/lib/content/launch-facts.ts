...
export const launchTruth = {
  ...
  facts: {
    ...
    license: {
      ...
      status: "resolved",
      value: "In force",
      note: "Ledgerful (the engine and dashboard) is source-available under the PolyForm Noncommercial License 1.0.0 plus the Ledgerful Small-Entity Commercial Exception v1.0, which permits qualifying entities under US$1M aggregated gross revenue (You + Affiliates, Internal Business Use) to use Ledgerful internally at no charge, plus limited Evaluation Use and a 90-day transition grant as defined in the Exception. Introductory Commercial License pricing for broader Internal Business Use is published on /editions (provisional headcount bands). Hosted and Enterprise prices are not announced. The license is in force: Ledgerful, LLC formed (FL, effective 2026-07-01, doc L26000356073); individual→LLC IP assignment executed; COMMERCIAL-EXCEPTION.md counsel-reviewed; FL + USPTO trademark search clear. This web repository is separate and proprietary (all rights reserved, no license granted).",
    ...
    mcpPackage: {
      ...
      note: "@ledgerful/mcp-server is published on the public npm registry at version 0.1.23 (engine pin v0.2.14). The GitHub release MCP tarball (ledgerful-mcp-server-0.1.23.tgz) matches the npm package version. npx and npm install instructions are actionable.",
    ...
    release: {
      ...
      status: "resolved",
      value: "v0.2.14 with SHA-256 checksums, SBOM, and cosign signing",
      note: "The v0.2.14 release is publicly available on GitHub with binary archives for 4 platforms, SHA-256 checksums for each, CycloneDX SBOM (engine + MCP), cosign keyless signing (Sigstore Fulcio), SLSA build-provenance attestation, SBOM attestation, and cargo-auditable dependency embedding. Homebrew tap and Scoop bucket remotes were measured on 0.2.13 at claim-currency write (bump-manifests may lag). Community winget (winget install Ledgerful.Ledgerful) was measured on 0.2.13 (winget search 2026-09-18); do not claim 0.2.14 until search shows it. History: older 0.2.12 was microsoft/winget-pkgs#430322 merged 2026-09-06. History: older leftover 0.2.8 (#415913) merged; older leftover 0.2.9 (#416853) closed-superseded; older 0.2.11 was #423248. The community index can lag a merge by minutes–hours.",
      tag: "v0.2.14",
      localSourceVersion: "0.2.14",
      apiUrl:
    ...
    supplyChainAttestation: {
      ...
      status: "resolved",
      value: "SBOM, cosign signing, SLSA provenance shipped with v0.1.8+ (current: v0.2.14)",

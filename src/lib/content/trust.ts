...
export const releaseVerificationSteps: string[] = [
  ...
  `Note: download URLs are live on the ${launchTruth.facts.release.tag} GitHub Release. Windows Authenticode signing and macOS Developer ID / Gatekeeper notarization are not yet implemented — binaries may trigger OS security prompts on first launch. OS code signing is a planned enhancement. Supply chain attestation (SBOM, cosign, SLSA) shipped since v0.1.8 and continues in ${launchTruth.facts.release.tag} — see the supply chain section below.`,
];
...
export const productSubprocessors: ProductSubprocessor[] = [
  ...
  },
  {
    name: "GitHub",
    purpose:
      "Future GitHub App installation records and webhooks for the hosted control plane. Separate from the current public self-hosted GitHub Action (Ryan-AI-Studios/ledgerful-action), which runs in the customer's own repository CI.",
    maturity: "planned",
    deployment: "hosted",
    tier: "hosted-planned",
  },
...
export const proveClaims: ProveDontClaim[] = [
  {
    ...
    body:
      ...
      "(engine signing basis v2; track 0072)",
  },
  {
